import json
import subprocess
import threading
from pathlib import Path
from tkinter import filedialog

import customtkinter as ctk
from tkinterdnd2 import DND_FILES, TkinterDnD

ctk.set_appearance_mode("light")
ctk.set_default_color_theme("blue")

C_PRIMARY  = "#8479C4"
C_PRIM_H   = "#7268B3"
C_AMBER    = "#D4A043"
C_AMBER_H  = "#BE8F38"
C_GREEN    = "#5BA87E"
C_GREEN_H  = "#4A9070"
C_YTDL     = "#E06B6B"
C_YTDL_H   = "#C85C5C"
C_BG       = "#F9F8FF"
C_CARD     = "#FFFFFF"
C_SEP      = "#EAE7F8"
C_DROPBG   = "#F2F0FC"
C_DROPBRD  = "#C8C1EC"
C_GRAY     = "#7B7D8C"
C_TEXT     = "#3D3F4E"

WHISPER_MODELS = ["turbo", "large-v3", "medium", "small", "base", "tiny"]
LANGUAGES      = ["ru", "en", "auto"]
BITRATES       = ["64k", "48k", "96k", "128k"]


class App(ctk.CTk, TkinterDnD.DnDWrapper):
    def __init__(self):
        super().__init__()
        self.TkdndVersion = TkinterDnD._require(self)
        self.title("ТвойMeet")
        self.geometry("640x900")
        self.resizable(True, True)
        self.minsize(600, 500)
        self.configure(fg_color=C_BG)

        self.source_file:    str | None = None
        self.processed_file: str | None = None
        self.output_dir:     str | None = None
        self.json_path:      str | None = None

        self._build_ui()

    # ── UI ──────────────────────────────────────────────────────────────────

    def _build_ui(self):
        # Шапка — всегда видна, вне скролла
        header = ctk.CTkFrame(self, fg_color=C_PRIMARY, corner_radius=0, height=60)
        header.pack(fill="x", side="top")
        header.pack_propagate(False)
        ctk.CTkLabel(
            header, text="⚡ ТвойMeet",
            font=ctk.CTkFont(size=22, weight="bold"),
            text_color="#FFFFFF",
        ).pack(side="left", padx=22, pady=12)
        ctk.CTkLabel(
            header, text="транскрибация аудио и видео",
            font=ctk.CTkFont(size=12),
            text_color="#D4CEFF",
        ).pack(side="left", pady=16)

        # Скроллируемая область — всё остальное
        s = ctk.CTkScrollableFrame(self, fg_color=C_BG, scrollbar_button_color=C_DROPBRD)
        s.pack(fill="both", expand=True)
        self._s = s  # используется в _section и _sep

        P = {"padx": 20, "pady": 5}

        # ── Источник ──
        self._section("Источник")
        self.drop_frame = ctk.CTkFrame(
            s, fg_color=C_DROPBG, corner_radius=10,
            border_width=1, border_color=C_DROPBRD,
        )
        self.drop_frame.pack(fill="x", padx=20, pady=(2, 6))
        self.drop_label = ctk.CTkLabel(
            self.drop_frame,
            text="⬇  Перетащи файл сюда",
            font=ctk.CTkFont(size=14),
            text_color=C_PRIMARY, height=64,
        )
        self.drop_label.pack(expand=True)
        self.drop_label.drop_target_register(DND_FILES)
        self.drop_label.dnd_bind("<<Drop>>", self._on_drop)
        self.drop_frame.drop_target_register(DND_FILES)
        self.drop_frame.dnd_bind("<<Drop>>", self._on_drop)

        file_row = ctk.CTkFrame(s, fg_color="transparent")
        file_row.pack(fill="x", padx=20, pady=(0, 4))
        ctk.CTkButton(
            file_row, text="Открыть файл", command=self._open_file,
            fg_color=C_PRIMARY, hover_color=C_PRIM_H, width=140,
        ).pack(side="left")
        self.file_label = ctk.CTkLabel(
            file_row, text="файл не выбран",
            text_color=C_GRAY, font=ctk.CTkFont(size=12),
        )
        self.file_label.pack(side="left", padx=12)

        self._sep()

        # ── Скачать по ссылке ──
        self._section("Скачать аудио по ссылке")
        yt_row = ctk.CTkFrame(s, fg_color="transparent")
        yt_row.pack(fill="x", padx=20, pady=(2, 6))
        self.yt_entry = ctk.CTkEntry(
            yt_row, placeholder_text="https://...",
            fg_color=C_CARD, border_color=C_DROPBRD, text_color=C_TEXT,
        )
        self.yt_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))
        ctk.CTkButton(
            yt_row, text="▼ Скачать", command=self._run_ytdlp,
            fg_color=C_YTDL, hover_color=C_YTDL_H, width=110,
        ).pack(side="left")

        self._sep()

        # ── FFmpeg ──
        self._section("FFmpeg — извлечение аудио")
        r1 = ctk.CTkFrame(s, fg_color="transparent")
        r1.pack(fill="x", padx=20, pady=(2, 4))
        ctk.CTkLabel(r1, text="Битрейт:", text_color=C_GRAY).pack(side="left")
        self.bitrate_var = ctk.StringVar(value="64k")
        ctk.CTkOptionMenu(
            r1, values=BITRATES, variable=self.bitrate_var, width=90,
            fg_color=C_CARD, button_color=C_PRIMARY, button_hover_color=C_PRIM_H,
            text_color=C_TEXT, dropdown_text_color=C_TEXT,
        ).pack(side="left", padx=(6, 24))
        ctk.CTkLabel(r1, text="Формат:", text_color=C_GRAY).pack(side="left")
        self.audio_fmt_var = ctk.StringVar(value="mp3")
        ctk.CTkOptionMenu(
            r1, values=["mp3", "wav"], variable=self.audio_fmt_var, width=90,
            fg_color=C_CARD, button_color=C_PRIMARY, button_hover_color=C_PRIM_H,
            text_color=C_TEXT, dropdown_text_color=C_TEXT,
        ).pack(side="left", padx=6)

        cut_row = ctk.CTkFrame(s, fg_color="transparent")
        cut_row.pack(fill="x", padx=20, pady=(6, 2))
        self.cut_mode = ctk.StringVar(value="full")
        ctk.CTkRadioButton(
            cut_row, text="Весь файл",
            variable=self.cut_mode, value="full",
            command=self._toggle_cut,
            fg_color=C_PRIMARY, hover_color=C_PRIM_H, text_color=C_TEXT,
        ).pack(side="left", padx=(0, 24))
        ctk.CTkRadioButton(
            cut_row, text="Только фрагмент",
            variable=self.cut_mode, value="fragment",
            command=self._toggle_cut,
            fg_color=C_PRIMARY, hover_color=C_PRIM_H, text_color=C_TEXT,
        ).pack(side="left")

        self.cut_frame = ctk.CTkFrame(s, fg_color="transparent")
        self.cut_frame.pack(fill="x", padx=20, pady=2)
        ctk.CTkLabel(self.cut_frame, text="От:", text_color=C_GRAY).pack(side="left")
        self.start_entry = ctk.CTkEntry(
            self.cut_frame, placeholder_text="00:00:00", width=105,
            border_color=C_DROPBRD, fg_color=C_CARD, text_color=C_TEXT,
        )
        self.start_entry.pack(side="left", padx=(6, 20))
        ctk.CTkLabel(self.cut_frame, text="До:", text_color=C_GRAY).pack(side="left")
        self.end_entry = ctk.CTkEntry(
            self.cut_frame, placeholder_text="00:18:00", width=105,
            border_color=C_DROPBRD, fg_color=C_CARD, text_color=C_TEXT,
        )
        self.end_entry.pack(side="left", padx=6)
        self._toggle_cut()

        ctk.CTkButton(
            s, text="▶  Обработать FFmpeg",
            command=self._run_ffmpeg,
            fg_color=C_AMBER, hover_color=C_AMBER_H,
            text_color="#1a1a1a", font=ctk.CTkFont(weight="bold"),
        ).pack(**P)

        self._sep()

        # ── Whisper ──
        self._section("Whisper — транскрибация")
        r2 = ctk.CTkFrame(s, fg_color="transparent")
        r2.pack(fill="x", padx=20, pady=(2, 4))
        ctk.CTkLabel(r2, text="Модель:", text_color=C_GRAY).pack(side="left")
        self.model_var = ctk.StringVar(value="turbo")
        ctk.CTkOptionMenu(
            r2, values=WHISPER_MODELS, variable=self.model_var, width=130,
            fg_color=C_CARD, button_color=C_PRIMARY, button_hover_color=C_PRIM_H,
            text_color=C_TEXT, dropdown_text_color=C_TEXT,
        ).pack(side="left", padx=(6, 24))
        ctk.CTkLabel(r2, text="Язык:", text_color=C_GRAY).pack(side="left")
        self.lang_var = ctk.StringVar(value="ru")
        ctk.CTkOptionMenu(
            r2, values=LANGUAGES, variable=self.lang_var, width=95,
            fg_color=C_CARD, button_color=C_PRIMARY, button_hover_color=C_PRIM_H,
            text_color=C_TEXT, dropdown_text_color=C_TEXT,
        ).pack(side="left", padx=6)

        ctk.CTkButton(
            s, text="▶  Транскрибировать",
            command=self._run_whisper,
            fg_color=C_PRIMARY, hover_color=C_PRIM_H,
            font=ctk.CTkFont(weight="bold"),
        ).pack(**P)

        self._sep()

        # ── Форматы ──
        self._section("Форматы вывода")
        fmt_row = ctk.CTkFrame(s, fg_color="transparent")
        fmt_row.pack(fill="x", padx=20, pady=(2, 4))
        self.fmt_vars: dict[str, ctk.BooleanVar] = {}
        for fmt in ["txt", "json", "srt", "vtt", "tsv"]:
            v = ctk.BooleanVar(value=(fmt in {"txt", "json", "srt"}))
            self.fmt_vars[fmt] = v
            ctk.CTkCheckBox(
                fmt_row, text=fmt, variable=v, width=68,
                fg_color=C_PRIMARY, hover_color=C_PRIM_H,
                checkmark_color="#FFFFFF", text_color=C_TEXT,
            ).pack(side="left", padx=4)

        self._sep()

        ctk.CTkButton(
            s, text="📄  Собрать .md для LLM",
            command=self._build_md,
            fg_color=C_GREEN, hover_color=C_GREEN_H,
            font=ctk.CTkFont(size=14, weight="bold"),
            height=40,
        ).pack(padx=20, pady=6, fill="x")

        self._sep()

        # ── Лог ──
        self._section("Лог")
        self.log_box = ctk.CTkTextbox(
            s, height=160,
            font=ctk.CTkFont(family="Courier New", size=11),
            fg_color=C_CARD, border_color=C_SEP, border_width=1,
            text_color=C_TEXT, state="disabled",
        )
        self.log_box.pack(fill="x", padx=20, pady=(2, 20))

    def _section(self, text: str):
        ctk.CTkLabel(
            self._s, text=text,
            font=ctk.CTkFont(size=12, weight="bold"),
            text_color=C_GRAY,
        ).pack(anchor="w", padx=20, pady=(6, 2))

    def _sep(self):
        ctk.CTkFrame(self._s, height=1, fg_color=C_SEP).pack(fill="x", padx=20, pady=5)

    # ── Helpers ─────────────────────────────────────────────────────────────

    def _log(self, msg: str):
        self.log_box.configure(state="normal")
        self.log_box.insert("end", msg + "\n")
        self.log_box.see("end")
        self.log_box.configure(state="disabled")

    def _toggle_cut(self):
        s = "normal" if self.cut_mode.get() == "fragment" else "disabled"
        self.start_entry.configure(state=s)
        self.end_entry.configure(state=s)

    # ── File input ──────────────────────────────────────────────────────────

    def _on_drop(self, event):
        path = event.data.strip().strip("{}")
        self._set_file(path)

    def _open_file(self):
        path = filedialog.askopenfilename(
            filetypes=[
                ("Медиафайлы", "*.mp4 *.mkv *.avi *.mov *.mp3 *.wav *.m4a *.flac *.ogg *.webm"),
                ("Все файлы", "*.*"),
            ]
        )
        if path:
            self._set_file(path)

    def _set_file(self, path: str):
        self.source_file    = path
        self.processed_file = None
        self.json_path      = None
        name = Path(path).name
        self.file_label.configure(text=name, text_color=C_GREEN)
        self.drop_label.configure(text=f"✓  {name}", text_color=C_GREEN)
        self._log(f"> Файл: {path}")

    # ── yt-dlp ──────────────────────────────────────────────────────────────

    def _run_ytdlp(self):
        url = self.yt_entry.get().strip()
        if not url:
            self._log("! Вставь ссылку")
            return
        threading.Thread(target=self._ytdlp_worker, args=(url,), daemon=True).start()

    def _ytdlp_worker(self, url: str):
        out_dir = Path.home() / "Desktop" / "ai-lab"
        out_tpl = str(out_dir / "source.%(ext)s")
        cmd = ["yt-dlp", "-x", "--audio-format", "mp3", "--audio-quality", "5", "-o", out_tpl, url]

        self._log("> yt-dlp: скачиваю аудио...")
        res = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8", errors="replace")

        if res.returncode == 0:
            out_file = out_dir / "source.mp3"
            if out_file.exists():
                self._set_file(str(out_file))
                self._log("✓ Скачано: source.mp3")
            else:
                mp3s = sorted(out_dir.glob("*.mp3"), key=lambda f: f.stat().st_mtime, reverse=True)
                if mp3s:
                    self._set_file(str(mp3s[0]))
                    self._log(f"✓ Скачано: {mp3s[0].name}")
                else:
                    self._log("! Файл не найден после скачивания")
        else:
            self._log(f"! Ошибка yt-dlp:\n" + "\n".join(res.stderr.splitlines()[-6:]))

    # ── FFmpeg ──────────────────────────────────────────────────────────────

    def _run_ffmpeg(self):
        if not self.source_file:
            self._log("! Выбери файл или скачай по ссылке")
            return
        threading.Thread(target=self._ffmpeg_worker, daemon=True).start()

    def _ffmpeg_worker(self):
        src     = Path(self.source_file)
        fmt     = self.audio_fmt_var.get()
        bitrate = self.bitrate_var.get()
        out     = src.parent / f"{src.stem}_audio.{fmt}"

        cmd = ["ffmpeg", "-y"]
        if self.cut_mode.get() == "fragment":
            start = self.start_entry.get().strip() or "00:00:00"
            end   = self.end_entry.get().strip()
            cmd  += ["-ss", start]
            if end:
                cmd += ["-to", end]
        cmd += ["-i", str(src), "-vn"]
        cmd += ["-c:a", "libmp3lame", "-b:a", bitrate] if fmt == "mp3" else ["-c:a", "pcm_s16le"]
        cmd.append(str(out))

        self._log("> FFmpeg запущен...")
        res = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8", errors="replace")
        if res.returncode == 0:
            self.processed_file = str(out)
            self._log(f"✓ Аудио готово: {out.name}")
        else:
            self._log("! Ошибка FFmpeg:\n" + "\n".join(res.stderr.splitlines()[-8:]))

    # ── Whisper ─────────────────────────────────────────────────────────────

    def _run_whisper(self):
        target = self.processed_file or self.source_file
        if not target:
            self._log("! Сначала выбери файл")
            return
        threading.Thread(target=self._whisper_worker, args=(target,), daemon=True).start()

    def _whisper_worker(self, audio_path: str):
        try:
            from faster_whisper import WhisperModel
        except ImportError:
            self._log("! faster-whisper не установлен: pip install faster-whisper")
            return

        model_name = self.model_var.get()
        lang = self.lang_var.get() or None
        if lang == "auto":
            lang = None

        audio   = Path(audio_path)
        out_dir = audio.parent / f"transcripts_{audio.stem}"
        out_dir.mkdir(exist_ok=True)
        self.output_dir = str(out_dir)

        self._log(f"> Загружаю модель {model_name}... (первый раз — скачивается)")
        try:
            model = WhisperModel(model_name, device="cpu", compute_type="int8")
            self._log(f"> Транскрибирую: {audio.name}")
            segs_raw, info = model.transcribe(audio_path, language=lang, beam_size=5)

            segs, texts = [], []
            for seg in segs_raw:
                t = seg.text.strip()
                segs.append({"start": seg.start, "end": seg.end, "text": t})
                texts.append(t)
                self._log(f"  [{seg.start:.0f}s] {t[:70]}")

            base     = out_dir / audio.stem
            selected = {f for f, v in self.fmt_vars.items() if v.get()}

            if "txt" in selected:
                (base.with_suffix(".txt")).write_text("\n".join(texts), encoding="utf-8")

            json_data = {"language": info.language, "duration": info.duration,
                         "text": " ".join(texts), "segments": segs}
            if "json" in selected:
                jf = base.with_suffix(".json")
                jf.write_text(json.dumps(json_data, ensure_ascii=False, indent=2), encoding="utf-8")
                self.json_path = str(jf)

            if "srt" in selected:
                lines = []
                for i, s in enumerate(segs, 1):
                    lines += [str(i), f"{_srt_t(s['start'])} --> {_srt_t(s['end'])}", s["text"], ""]
                (base.with_suffix(".srt")).write_text("\n".join(lines), encoding="utf-8")

            if "vtt" in selected:
                lines = ["WEBVTT", ""]
                for s in segs:
                    lines += [f"{_vtt_t(s['start'])} --> {_vtt_t(s['end'])}", s["text"], ""]
                (base.with_suffix(".vtt")).write_text("\n".join(lines), encoding="utf-8")

            if "tsv" in selected:
                lines = ["start\tend\ttext"] + [
                    f"{s['start']:.3f}\t{s['end']:.3f}\t{s['text']}" for s in segs]
                (base.with_suffix(".tsv")).write_text("\n".join(lines), encoding="utf-8")

            self._log(f"✓ Готово → {out_dir}")
        except Exception as e:
            self._log(f"! Ошибка Whisper: {e}")

    # ── MD builder ──────────────────────────────────────────────────────────

    def _build_md(self):
        json_p = self.json_path
        if not json_p and self.output_dir:
            found = list(Path(self.output_dir).glob("*.json"))
            if found:
                json_p = str(found[0])
        if not json_p:
            self._log("! Нет JSON — сначала транскрибируй")
            return
        try:
            data     = json.loads(Path(json_p).read_text(encoding="utf-8"))
            out      = Path(json_p).with_suffix(".md")
            out.write_text(build_md_text(data, Path(json_p).stem), encoding="utf-8")
            self._log(f"✓ MD готов: {out.name}")
        except Exception as e:
            self._log(f"! Ошибка: {e}")


# ── Pure functions (testable without UI) ─────────────────────────────────────

def _srt_t(s: float) -> str:
    h   = int(s // 3600)
    m   = int((s % 3600) // 60)
    sec = s % 60
    return f"{h:02d}:{m:02d}:{int(sec):02d},{round((sec % 1) * 1000):03d}"


def _vtt_t(s: float) -> str:
    return _srt_t(s).replace(",", ".")


def build_ffmpeg_cmd(src: str, fmt: str, bitrate: str,
                     cut: bool, start: str, end: str) -> list[str]:
    out = str(Path(src).parent / f"{Path(src).stem}_audio.{fmt}")
    cmd = ["ffmpeg", "-y"]
    if cut:
        cmd += ["-ss", start or "00:00:00"]
        if end:
            cmd += ["-to", end]
    cmd += ["-i", src, "-vn"]
    cmd += ["-c:a", "libmp3lame", "-b:a", bitrate] if fmt == "mp3" else ["-c:a", "pcm_s16le"]
    cmd.append(out)
    return cmd


def build_md_text(data: dict, stem: str) -> str:
    text     = data.get("text", "")
    lang     = data.get("language", "?")
    duration = data.get("duration", 0)
    return f"""# Транскрипт: {stem}

**Язык:** {lang}
**Длительность:** {duration / 60:.1f} мин

---

## Полная транскрибация

{text}

---

## Prompt для LLM

Перед тобой полная транскрибация аудио.

Сделай следующее:

1. Главное в одном абзаце
2. Ключевые идеи
3. Сильные формулировки
4. Что можно использовать для инфографики
5. Что можно выкинуть

Формат строго Markdown. Не добавляй факты, которых нет в материале.
"""


if __name__ == "__main__":
    app = App()
    app.mainloop()
