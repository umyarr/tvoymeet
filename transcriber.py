...
class App(ctk.CTk, TkinterDnD.DnDWrapper):
    ...
    def _build_ui(self):
        ...
        self.drop_label = ctk.CTkLabel(
            ...
            text_color=C_PRIMARY, height=60,
        )
        ...
        file_row = ctk.CTkFrame(c1, fg_color="transparent")
        ...
        # ── Card 2: Скачать по ссылке ────────────────────────────────────────
        c2 = self._card(s)
        self._card_title(c2, "2. Скачать аудио по ссылке")

        yt_row = ctk.CTkFrame(c2, fg_color="transparent")
        yt_row.pack(fill="x")
        ...
        ctk.CTkButton(
            ...
            fg_color=C_YTDL, hover_color=C_YTDL_H, width=110, height=36,
        ).pack(side="left")

        # ── Card 3: FFmpeg ───────────────────────────────────────────────────
        c3 = self._card(s)
        self._card_title(c3, "3. FFmpeg — извлечение аудио")

        r1 = ctk.CTkFrame(c3, fg_color="transparent")
        r1.pack(fill="x", pady=(0, 8))
        ctk.CTkLabel(r1, text="Битрейт:", text_color=C_GRAY,
                     font=ctk.CTkFont(size=12)).pack(side="left")
        self.bitrate_var = ctk.StringVar(value="64k")
        ctk.CTkOptionMenu(
            r1, values=BITRATES, variable=self.bitrate_var, width=90, height=34,
        ...
        ctk.CTkLabel(r1, text="Формат:", text_color=C_GRAY,
                     font=ctk.CTkFont(size=12)).pack(side="left")
        self.audio_fmt_var = ctk.StringVar(value="mp3")
        ctk.CTkOptionMenu(
            r1, values=["mp3", "wav"], variable=self.audio_fmt_var, width=90, height=34,
            fg_color=C_CARD, button_color=C_PRIMARY, button_hover_color=C_PRIM_H,
            text_color=C_TEXT, dropdown_text_color=C_TEXT,
        ).pack(side="left", padx=6)

        cut_row = ctk.CTkFrame(c3, fg_color="transparent")
        ...
        self.cut_frame = ctk.CTkFrame(c3, fg_color="transparent")
        self.cut_frame.pack(fill="x", pady=(0, 10))
        ctk.CTkLabel(self.cut_frame, text="От:", text_color=C_GRAY,
                     font=ctk.CTkFont(size=12)).pack(side="left")
        self.start_entry = ctk.CTkEntry(
            self.cut_frame, placeholder_text="00:00:00", width=105, height=34,
            border_color=C_DROPBRD, fg_color=C_CARD, text_color=C_TEXT,
        )
        self.start_entry.pack(side="left", padx=(6, 20))
        ctk.CTkLabel(self.cut_frame, text="До:", text_color=C_GRAY,
                     font=ctk.CTkFont(size=12)).pack(side="left")
        self.end_entry = ctk.CTkEntry(
            self.cut_frame, placeholder_text="00:18:00", width=105, height=34,
            border_color=C_DROPBRD, fg_color=C_CARD, text_color=C_TEXT,
        )
        ...
        # ── Card 4: Whisper ──────────────────────────────────────────────────
        c4 = self._card(s)
        self._card_title(c4, "4. Whisper — транскрибация")

        r2 = ctk.CTkFrame(c4, fg_color="transparent")
        r2.pack(fill="x", pady=(0, 10))
        ctk.CTkLabel(r2, text="Модель:", text_color=C_GRAY,
                     font=ctk.CTkFont(size=12)).pack(side="left")
        self.model_var = ctk.StringVar(value="turbo")
        ctk.CTkOptionMenu(
            r2, values=WHISPER_MODELS, variable=self.model_var, width=130, height=34,
            fg_color=C_CARD, button_color=C_PRIMARY, button_hover_color=C_PRIM_H,
            text_color=C_TEXT, dropdown_text_color=C_TEXT,
        ).pack(side="left", padx=(6, 24))
        ctk.CTkLabel(r2, text="Язык:", text_color=C_GRAY,
                     font=ctk.CTkFont(size=12)).pack(side="left")
        self.lang_var = ctk.StringVar(value="ru")
        ctk.CTkOptionMenu(
            r2, values=LANGUAGES, variable=self.lang_var, width=95, height=34,
            fg_color=C_CARD, button_color=C_PRIMARY, button_hover_color=C_PRIM_H,
            text_color=C_TEXT, dropdown_text_color=C_TEXT,
        ).pack(side="left", padx=6)

        # Форматы вывода внутри той же карточки
        ctk.CTkLabel(c4, text="Форматы вывода:", text_color=C_GRAY,
                     font=ctk.CTkFont(size=12)).pack(anchor="w", pady=(0, 6))
        fmt_row = ctk.CTkFrame(c4, fg_color="transparent")
        ...
        # ── MD Button ────────────────────────────────────────────────────────
        md_card = self._card(s)
        ctk.CTkButton(
            md_card, text="Собрать .md для LLM",
